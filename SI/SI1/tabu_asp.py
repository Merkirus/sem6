...
if __name__ == '__main__':
    ...
    def diff_set(prev_neighbour, neighbour):
        if len(prev_neighbour) != len(neighbour):
            return {}

        l = [x.value for x, y in zip(prev_neighbour, neighbour) if x != y]

        return set(l)


    def aspiration_criteria(k, best_solution, solution, opt, time, g):
        neighbour_cost, _, _ = get_solution_cost(solution, opt, time, g)
        number_of_changes = len(diff_set(best_solution, solution))
        return neighbour_cost + 0.5 * (k - number_of_changes)
    ...
    def tabu_search(start, stops, opt, time, tabuList_size):
        ...
        for k in range(1000):
            ...
            prev_neighbour = solution

            for neighbour in neighbours:
                diff = diff_set(prev_neighbour, neighbour)

                if diff in tabuList and aspiration_criteria(k, best_neighbour, neighbour, opt, time, g) > best_neighbour_cost:
                    continue
